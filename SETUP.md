...
2. **Add navigation** in `src/lib/components/Navbar.svelte`:
   ...
   <a
   	href="/my-page"
   	class="text-surface-700 hover:text-surface-900 dark:text-surface-300 dark:hover:text-surface-100"
   >
   	My Page
   </a>
   ```
...
```svelte
<TransactionButton onClick={handleTransaction}>Send Transaction</TransactionButton>
```
